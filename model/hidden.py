...
from model.factory import get_encoder_decoder_dis
...
class Hidden:
    def __init__(self, configuration: HiDDenConfiguration, device: torch.device, noiser: Noiser, tb_logger):
        ...
        encoder_decoder,discriminator = get_encoder_decoder_dis(configuration.encoder_decoder_name,configuration, noiser)
    ...
    def train_on_batch(self, batch: list,var,encoder_weight, decoder_weight):
        ...
        with torch.enable_grad():
            ...
            # train on fake
            images, encoded_images,discriminator_images,discriminator_encoded_images, noised_images, decoded_messages = self.encoder_decoder(images, messages,var,batch_size)
            #encoded_images, noised_images, decoded_messages = self.encoder_decoder(images, messages)

            d_on_cover = self.discriminator(discriminator_images)
            d_loss_on_cover = self.bce_with_logits_loss(d_on_cover, d_target_label_cover)
            d_loss_on_cover.backward()



            d_on_encoded = self.discriminator(discriminator_encoded_images.detach())
            ...
            d_on_encoded_for_enc = self.discriminator(discriminator_encoded_images)
    ...
    def validate_on_batch(self, batch: list,var,encoder_weight, decoder_weight):
        ...
        with torch.no_grad():
            # Change here the type of these data
            d_target_label_cover = torch.full((batch_size, 1), self.cover_label, device=self.device, dtype=torch.float)
            d_target_label_encoded = torch.full((batch_size, 1), self.encoded_label, device=self.device, dtype=torch.float)
            g_target_label_encoded = torch.full((batch_size, 1), self.cover_label, device=self.device, dtype=torch.float)
            ...
            # change the images, since the embedding and image
            images, encoded_images,discriminator_images,discriminator_encoded_images, noised_images, decoded_messages = self.encoder_decoder(images, messages,var,batch_size)
            #encoded_images, noised_images, decoded_messages = self.encoder_decoder(images, messages)
            ...
            d_on_encoded = self.discriminator(discriminator_encoded_images)
            d_loss_on_encoded = self.bce_with_logits_loss(d_on_encoded, d_target_label_encoded)
