...
# sample
    ...
    with torch.inference_mode():
        with torch.autocast('cuda', enabled=True, dtype=torch.float16, cache_enabled=True):    # using bfloat16 can be faster
            ...
            recon_B3HW = var.vae_proxy[0].fhat_to_img(f_hats[-1]).add_(1).mul_(0.5) 


    for i in range(recon_B3HW.size(0)):  
        single_img = recon_B3HW[i]  # [3, H, W] 
        single_img = single_img.permute(1, 2, 0).mul(255).cpu().numpy()  #  0-255
        single_img = PImage.fromarray(single_img.astype(np.uint8))  # PIL
        file_name = osp.join(save_path, f"image{generated_count + i}.png")  
        single_img.save(file_name)
        print(f"Saved image tensor {generated_count + i} to {file_name}")
    
    generated_count += current_batch_size 
print(f"All image {total_samples} samples have been saved to {save_path}")